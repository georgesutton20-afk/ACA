...
/** Topic page body: static content (bundled) + the learner's own progress. */
export function TopicDetail({ topic, questionCount }: { topic: Topic; questionCount: number }) {
  const topicId = topic.id;
  const { userId } = useAuth();
  const { data: progressList } = useAsync(() => data.getProgress(userId!), [userId], !!userId);
  const progress = progressList?.find((p) => p.topicId === topicId);
          ...
          <Card className="animate-[rise_0.4s]">
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Progress value={mastery} indicatorClassName={mastery >= 80 ? "bg-success" : undefined} />
              </div>
